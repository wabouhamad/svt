...
# oc adm cordon {MASTER_INTERNAL_IP}
## NODES=$(oc get nodes | grep -v "SchedulingDisabled" | grep -v NAME | awk '{print $1}'  | xargs)
...
# python -u cluster-loader.py -vf config/pyconfigLoadedProject.yaml
...
for i in {0..9}; do
  oc get all -n clusterproject${i}
done

echo -e "\nSleeping for 10 mins"
sleep 600

echo -e "\nDeleting the 10 projects we just created"
for i in {0..9}; do
  oc delete project -n clusterproject${i}
done
